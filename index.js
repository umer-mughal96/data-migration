...
async function main() {
  mongoose
    .connect(
      "mongodb+srv://salesforcefreelancing96:169pSRT1smrDi6Wr@cluster0.map59fw.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0",
      {
        ...
        for (const plot of plotArray) {
          ...
          processSchema.Lines[0].LinesArray.forEach((line, index) => {
            if (line.Line.includes("Mobile")) {
              const mobile = processSchema.Lines[0].LinesArray[
                index
              ].Line.match(re)?.map(function (s) {
                return s.trim();
              });
              if (mobile?.length > 0) {
            ...
            if (
              line.Line.includes("Net Price :") ||
              line.Line.includes("Price :")
            ) {
              console.log(
                "netpppppprice",
                processSchema.Lines[0].LinesArray[index + 1].Line
              );
              schema.netPrice =
                processSchema.Lines[0].LinesArray[index + 1].Line;
            }
            ...
            if (
              line.Line.includes("Sub Total =>>") ||
              line.Line.includes("Sub Total")
            ) {
              subTotal.dueAmount =
                processSchema.Lines[0].LinesArray[index + 1].Line;
              subTotal.receieveAmount =
                processSchema.Lines[0].LinesArray[index + 3].Line;
              subTotal.adjustmentAmount =
                processSchema.Lines[0].LinesArray[index + 5].Line;
              schema.summary.subTotal = subTotal;
            }
            if (
              line.Line.includes("Grand Total =>>") ||
              line.Line.includes("Grand Total")
            ) {
              grandTotal.dueAmount =
                processSchema.Lines[0].LinesArray[index + 1].Line;
              grandTotal.receieveAmount =
                processSchema.Lines[0].LinesArray[index + 3].Line;
          ...
          console.log(
            "=======> STARTED MAINTAINING DATABASE OF REGISTRATION NUMBER ",
            schema
          );
          // return;

          land = await Land.findOne({
            registrationNo: plot.registrationNumber,
          });

          if (!land) {
            land = await Land.create({
              ...schema,
              subTotal: schema.summary.subTotal,
              grandTotal: schema.summary.grandTotal,
            });
          }
          if (schema.cnic) {
            currentOwner = await Com.create({
              ...schema,
              land: land._id,
              currentOwner: true,
            }); // This Could Be Optional
            historyIds.push(currentOwner._id);
          }
          if (schema.summary.payment.length > 0) {
            for (const booking of schema.summary.payment) {
              const bookin = await Booking.create({
                ...booking,
                land: land._id,
              });
          ...
          if (schema.history.length > 0) {
            for (const hist of schema.history) {
              const history = await Com.create({ ...hist, land: land._id });
